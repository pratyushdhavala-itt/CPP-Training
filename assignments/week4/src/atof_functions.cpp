...
int Atof::convertToInteger(const char* input) {
    bool isNegative = false;
    if (ExceptionHandling::checkForNegative(input)) {
        isNegative = true;
        input++;
    }
    double result = 0.0;
...
int Atof::calculateInputLength(const char* input) {
    int inputLength = 0;
...
double Atof::findPower(double base, double exponent) {
    double result = 1.0;
    if (exponent >= 0) {
...
void Atof::removeWhiteSpaces(char* input) {
    int startingIndex = 0;
...
int Atof::findPreDecimalPointLength(const char* input) {
...
int Atof::findPostDecimalPointLength(const char* input) {
    int inputLength = calculateInputLength(input);
...
double Atof::convertToFloatingNumber(const char* input) {
    bool isNegative = false;
    if (ExceptionHandling::checkForNegative(input)) {
        isNegative = true;
        input++;
...
int Atof::firstDecimalIndex(const char* input) {
    int index = 0;
    while (input[index] != '\0') {
        if (input[index] == '.') {
            return index;
...
double Atof::convertToNumber(const char* input) {
    double result = 0.0;
    if (ExceptionHandling::isValidFloatingPoint(input)) {
